...
UID_match = 0  # Проверка совпадения UIDdata_match= 0  # Проверка срока работы метки
date1 = xl1.iloc[0, 2]
day1 = str(date1)[8:10]
...
while True:
    ...
    if UID == UID1:
        ...
        UID_match = 1
        if data.tm_year < int(year3):
            data_match = 1
        elif data.tm_year == int(year3):
            if data.tm_mon < int(month3):
                data_match = 1
            elif data.tm_mon == int(month3):
                if data.tm_mday <= int(day3):
                    data_match = 1
